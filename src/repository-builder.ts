...
export class ActorRepositoryBuilder {
    static build(client: DynamoDB.DocumentClient, tableSuffix: string = 'v1'): ActorRepository {
        return new DynamoActorRepository(client, tableSuffix);
    }
}

export class ActorNameRepositoryBuilder {
    static build(client: DynamoDB.DocumentClient, tableSuffix: string = 'v1'): ActorNameRepository {
