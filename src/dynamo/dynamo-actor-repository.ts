...
export class DynamoActorRepository
  extends DynamoRepository<Actor>
  implements ActorRepository
{
  ...
  protected override beforeCreate(data: Actor): Actor {
  ...
  protected override beforeUpdate(data: RepositoryUpdateData<Actor>) {
    if (data.set) {
